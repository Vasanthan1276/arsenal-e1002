...
<div class="snapshot-grid">
  ...
  <div><small>MINI-LEAGUE</small><b>${mini ? `#${fmt(mini.rank)}` : '—'}</b><span>${mini ? esc(mini.name) : 'No league data'}</span></div>
...
<div class="snapshot-lines">
  <div><span>Captain</span><b>${mainCaptain ? esc(mainCaptain.name) : '—'}</b><strong>${mainCaptain ? `${fmt(mainCaptain.contribution ?? mainCaptain.gwPoints)} pts` : ''}</strong></div>
  <div><span>Best GW contribution</span><b>${mainBest ? esc(mainBest.name) : '—'}</b><strong>${mainBest ? `${fmt(mainBest.contribution ?? mainBest.gwPoints)} pts` : ''}</strong></div>
  <div><span>Data freshness</span><b>Football ${esc(football.updated || '—')}</b><strong>Fantasy ${esc(fantasy.updated || '—')}</strong></div>
</div>`;
...
<p class="footnote">Generated from the same GitHub data pipeline as the E1002 dashboard. Football: ${esc(football.updated || '—')} • Fantasy: ${esc(fantasy.updated || '—')}.</p>
